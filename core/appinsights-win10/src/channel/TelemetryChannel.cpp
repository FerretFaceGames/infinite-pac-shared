...
void TelemetryChannel::Send()
{
	if (m_buffer.size() > 0)
	{
		...
		//HANDLE hRespRecv = ::CreateEventEx(nullptr, nullptr, CREATE_EVENT_MANUAL_RESET, EVENT_MODIFY_STATE | SYNCHRONIZE);
		...
		//::WaitForSingleObject(hRespRecv, 4000);
		//::CloseHandle(hRespRecv);
	}
}
